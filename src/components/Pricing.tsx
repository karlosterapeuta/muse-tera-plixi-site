...
const Pricing = () => {
  ...
  const pixPayloadSemFidelidade = generatePixPayload({ pixKey, beneficiaryName: 'MUSETERA', city: 'RECIFE', amount: 69.90 });
  const pixPayload6Meses = generatePixPayload({ pixKey, beneficiaryName: 'MUSETERA', city: 'RECIFE', amount: 299.00 });
  const pixPayload12Meses = generatePixPayload({ pixKey, beneficiaryName: 'MUSETERA', city: 'RECIFE', amount: 478.80 });
  ...
  const plans = [
    { name: 'Sem Fidelidade 30 dias', price: 'R$ 69,90', period: '/mês', description: 'Flexibilidade máxima para seu negócio', features: ['Pacientes ilimitados', 'Agendamento avançado', 'Relatórios detalhados', 'Planos de tratamento', 'Suporte prioritário', '50GB de armazenamento'], cta: 'Assinar Plano', popular: false, refundGuarantee: true },
    { name: 'FIDELIDADE 6 Meses', price: 'R$ 49,90', period: '/mês', description: 'Economia de 29% com compromisso semestral', features: ['Pacientes ilimitados', 'Agendamento avançado', 'Relatórios detalhados', 'Planos de tratamento', 'Suporte prioritário', '50GB de armazenamento'], cta: 'Assinar Plano', popular: true, refundGuarantee: true },
    { name: 'FIDELIDADE 12 Meses', price: 'R$ 39,90', period: '/mês', description: 'Melhor custo-benefício com plano anual', features: ['Pacientes ilimitados', 'Agendamento avançado', 'Relatórios detalhados', 'Planos de tratamento', 'Suporte prioritário', '50GB de armazenamento'], cta: 'Assinar Plano', popular: false, refundGuarantee: true }
  ];
              ...
              <CardContent className="space-y-6">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    else if (plan.name === 'FIDELIDADE 12 Meses') { setPix12MesesModalOpen(true); setShowPixQRCode(null); }
                  }}
                >
                  {plan.cta}
                </Button>
        ...
        <Dialog open={pix6MesesModalOpen} onOpenChange={(open) => { setPix6MesesModalOpen(open); if (!open) setShowPixQRCode(null); }}>
          <DialogContent className="max-w-[95vw] sm:max-w-lg mx-4 bg-white border-gray-200">
            {showPixQRCode === '6meses' ? (
              ...
              <>
                <DialogHeader>
                  <DialogTitle className="text-xl sm:text-2xl font-bold text-center text-foreground">Escolha a forma de pagamento</DialogTitle>
